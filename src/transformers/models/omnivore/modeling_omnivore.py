...
@dataclass
class OmnivoreJointClassificationOutput(ModelOutput):
    """
    Output type of [`OmnivoreForJointClassification`].

    Args:
        image_loss (`torch.FloatTensor` of shape `(1,)`, *optional*, returned when `image_labels` are provided)):
            Total loss for class prediction of image
        video_loss (`torch.FloatTensor` of shape `(1,)`, *optional*, returned when `image_labels` are provided)):
            Total loss for class prediction of video
        rgbd_loss (`torch.FloatTensor` of shape `(1,)`, *optional*, returned when `image_labels` are provided)):
            Total loss for class prediction of rgbd
        image_logits (`torch.FloatTensor` of shape `(batch_size, num_queries, num_classes)`):
            Classification logits for all images.
        video_logits (`torch.FloatTensor` of shape `(batch_size, num_queries, num_classes)`):
            Classification logits for all videos.
        rgbd_logits (`torch.FloatTensor` of shape `(batch_size, num_queries, num_classes)`):
            Classification logits for all rgbd.
        hidden_states (`tuple(torch.FloatTensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            Tuple of `torch.FloatTensor` (one for the output of each layer of `OmnivoreSwinTransformer3DLayer`) of shape `(batch_size, sequence_length, hidden_size)`. 
            Hidden-states of the model at the output of each layer..
    """
    image_loss: Optional[torch.FloatTensor] = None
    video_loss: Optional[torch.FloatTensor] = None
    rgbd_loss: Optional[torch.FloatTensor] = None
    image_logits: torch.FloatTensor = None
    video_logits: torch.FloatTensor = None
    rgbd_logits: torch.FloatTensor = None
...
def window_partition(input_feature, window_size):
    ...
    input_feature = input_feature.view(
        batch_size, D // window_size[0], window_size[0],
        height // window_size[1], window_size[1],
        width // window_size[2], window_size[2],
        channels
    )
    windows = (
        input_feature.permute(0, 1, 3, 5, 2, 4, 6, 7).contiguous().view(-1, reduce(mul, window_size), channels)
    )
    return windows
...
def window_partition_image(input_feature, window_size):
    ...
    input_feature = input_feature.view(
        batch_size, height // window_size[1], window_size[1],
        width // window_size[2], window_size[2], channels
    )
...
def window_reverse(windows, windows_size, batch_size, D, height, width):
    input_feature = windows.view(
        batch_size, D // windows_size[0], height // windows_size[1],
        width // windows_size[2], windows_size[0], windows_size[1],
        windows_size[2], -1
    )
...
class OmnivoreWindowAttention3D(nn.Module):
    def __init__(self, dim, window_size, num_heads, qkv_bias=False, qk_scale=None, attention_dropout_rate=0.0, projection_dropout_rate=0.0):
    ...
    def forward(self, hidden_state, attention_mask=None):
        ...
        queries_keys_values = (self.queries_keys_values(hidden_state).reshape(batch_size, seq_len, 3, self.num_heads, channels // self.num_heads).permute(2, 0, 3, 1, 4)
        )
        ...
        if attention_mask is not None:
            ...
            attention = attention.view(batch_size // nW, nW, self.num_heads, seq_len, seq_len) + attention_mask.unsqueeze(1).unsqueeze(0)
...
class OmnivoreSwinTransformer3DLayer(nn.Module):
    def __init__(self, dim, num_heads, window_size=(2, 7, 7), shift_size=(0, 0, 0), mlp_ratio=4.0, qkv_bias=True, qk_scale=None,
            dropout_rate=0.0, attention_dropout_rate=0.0, drop_path_rate=0.0, act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        self.window_size = window_size
        ...
        self.attention = OmnivoreWindowAttention3D(dim, window_size=self.window_size, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
            attention_dropout_rate=attention_dropout_rate, projection_dropout_rate=dropout_rate)

        self.drop_path = OmnivoreDropPath(drop_path_rate) if drop_path_rate > 0.0 else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = OmnivoreMLP(in_features=dim, hidden_features=mlp_hidden_dim, drop=dropout_rate, act_layer=act_layer)
    ...
    def forward_before(self, hidden_state, attention_mask):
        ...
        x = F.pad(highlight, (0, 0, pad_l, pad_r, pad_t, pad_b, pad_d0, pad_d1))
        _, Dp, Hp, Wp, _ = hidden_state.shape
        # cyclic shift
        if any(i > 0 for i in shift_size):
            shifted_hidden_state = torch.roll(hidden_state, shifts=(-shift_size[0], -shift_size[1], -shift_size[2]), dims=(1, 2, 3))
        ...
        if any(i > 0 for i in shift_size):
            hidden_state = torch.roll(shifted_hidden_state, shifts=(shift_size[0], shift_size[1], shift_size[2]), dims=(1, 2, 3))
        else:
...
class OmnivoreSwinTransformerStage(nn.Module):
    def __init__(self, dim, depth, num_heads, window_size=(1, 7, 7), mlp_ratio=4.0, qkv_bias=False, qk_scale=None, dropout_rate=0.0,
        attention_dropout_rate=0.0, drop_path_rate=0.0, norm_layer=nn.LayerNorm, downsample=None):
        super().__init__()
        self.window_size = window_size
    ...
    def forward(self, hidden_state, use_checkpoint=False, height=None, width=None, use_seg=False):
        ...
        window_size, shift_size = get_window_size(
            (D, height, width ), self.window_size, self.shift_size
        )
        from einops import rearrange
        print("Debug Einops B1:", hidden_state.shape)
        hidden_state = rearrange(hidden_state, "b c d h w -> b d h w c")
        print("Debug Einops A1:", hidden_state.shape)
        ...
        print("Debug Einops B2:", hidden_state.shape)
        hidden_state = rearrange(hidden_state, "b d h w c -> b c d h w")
        print("Debug Einops A2:", hidden_state.shape)
        return hidden_state
...
class OmnivorePatchEmbeddings3D(nn.Module):
    ...
    def __init__(
        self, patch_size=(2, 4, 4), input_channels=3, embed_dim=96, norm_layer=None, additional_variable_channels=None,
    ):
        super().__init__()
...
class OmnivoreSwinTransformer3DModel(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.depth_mode is not None:
            ...
            if self.depth_mode in ["separate_d_tokens", "summed_rgb_d_tokens"]:
                depth_chans = 1
                assert (
                    self.depth_patch_embed_separate_params
        ...
        for stage in range(self.num_stages):
            stage_module = OmnivoreSwinTransformerStage(
                ...
                drop_path=dpr[sum(self.depths[:stage]) : sum(self.depths[: stage + 1])],
                norm_layer=self.norm_layer,
                downsample=OmnivorePatchMerging if stage < self.num_stages - 1 else None,
            )
            self.layers.append(stage_module)
    ...
    def forward(self, hidden_state, out_feat_keys = None, use_checkpoint = False, output_hidden_states = False, return_dict = True):
...
class OmnivoreModel(OmnivorePreTrainedModel):
    ...
    def forward(
        ...
        model_outputs = self.model(pixel_values)
        last_hidden_state = model_outputs[0]

        last_hidden_state = model_outputs[0]

        # global average pooling, (N, C, D, H, W) -> (N, C)
        pooled_output = last_hidden_state.mean([-3, -2, -1])

        if not return_dict:
            return (last_hidden_state, pooled_output) + model_outputs[1:]
...
class OmnivoreForJointClassification(OmnivorePreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    @add_code_sample_docstrings(
        ...
        output_type=OmnivoreJointClassificationOutput,
        config_class=_CONFIG_FOR_DOC,
    ...
    def forward(
        ...
        if pixel_input_type == "image":
            image_logits = self.image_classifier(sequence_output)
        
        if pixel_input_type == "video":
            video_logits = self.video_classifier(sequence_output)

        if pixel_input_type == "rgbd":
            rgbd_logits = self.rgbd_classifier(sequence_output)
        ...
        if labels is not None:
            ...
            if self.config.problem_type == "regression":
                ...
                if self.num_labels == 1:
                    if pixel_input_type == "image":
                        image_loss = loss_fct(image_logits.squeeze(), labels.squeeze())
                    if pixel_input_type == "video":
                        video_loss = loss_fct(video_logits.squeeze(), labels.squeeze())
                    if pixel_input_type == "rgbd":
                        rgbd_loss = loss_fct(rgbd_logits.squeeze(), labels.squeeze())
                else:
                    if pixel_input_type == "image":
                        image_loss = loss_fct(image_logits.squeeze(), labels.squeeze())
                    if pixel_input_type == "video":
                ...
                if pixel_input_type == "image":
                    image_loss = loss_fct(image_logits.view(-1, self.num_labels), labels.view(-1))
                if pixel_input_type == "video":
                    video_loss = loss_fct(video_logits.view(-1, self.num_labels), labels.view(-1))
                if pixel_input_type == "rgbd":
                    rgbd_loss = loss_fct(rgbd_logits.view(-1, self.num_labels), labels.view(-1))

            elif self.config.problem_type == "multi_label_classification":
                loss_fct = BCEWithLogitsLoss()
                if pixel_input_type == "image":
                    image_loss = loss_fct(image_logits, labels)
                if pixel_input_type == "video":
                    image_loss = loss_fct(video_logits, labels)
                if pixel_input_type == "rgbd":
                    image_loss = loss_fct(rgbd_logits, labels)
                

        if not return_dict:
            output = (image_logits, video_logits, rgbd_logits) + outputs[2:]
            return ((image_loss, video_loss, rgbd_loss) + output) if loss is not None else output

        return OmnivoreJointClassificationOutput(
            image_loss=image_loss,
            video_loss=video_loss,
            rgbd_loss=rgbd_loss,
            image_logits=image_logits,
            video_logits=video_logits,
            rgbd_logits=rgbd_logits,
