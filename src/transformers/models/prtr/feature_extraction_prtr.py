...
"""Feature extractor class for PRTR."""
...
class PrtrFeatureExtractor(FeatureExtractionMixin, ImageFeatureExtractionMixin):
    r"""
    Constructs a PRTR feature extractor.
    ...
    def _is_valid_format(self, format):
        if format not in ["coco_detection", "mii_detection"]:
            raise ValueError(f"Format {format} not supported")
        return format

    def prepare(self, image, target, meta, masks_path=None):
        if self.format == "coco_detection":
            image, target = self.prepare_coco_detection(image, target, meta)
            return image, target
        elif self.format == "mii_detection":
            image, target = self.prepare_mii_detection(image, target, meta)
            return image, target
        else:
    ...
    def prepare_coco_detection(self, image, target, return_segmentation_masks=False):
        """
        Convert the target in COCO format into the format expected by PRTR.
        """
        w, h = image.size
        ...
        **kwargs,
        ...
        Args:
            ...
            annotations (`Dict`, `List[Dict]`, *optional*):
                ...
                In case [`PrtrFeatureExtractor`] was initialized with `format = "coco_detection"`, the annotations for
                ...
                In case [`PrtrFeatureExtractor`] was initialized with `format = "coco_panoptic"`, the annotations for
            ...
            masks_path (`pathlib.Path`, *optional*):
                ...
                relevant in case [`PrtrFeatureExtractor`] was initialized with `format = "coco_panoptic"`.
        ...
        # prepare (COCO annotations as a list of Dict -> PRTR target as a single Dict per image)
        if annotations is not None:
    ...
    # inspired by https://github.com/facebookresearch/detr/blob/master/models/prtr.py#L258
    def post_process(self, outputs, target_sizes):
        """
        Converts the output of [`PrtrForObjectDetection`] into the format expected by the COCO api. Only supports
        PyTorch.

        Args:
            outputs ([`PrtrObjectDetectionOutput`]):
