...
""" Testing suite for the PyTorch PRTR model. """
...
from transformers import PrtrConfig, is_timm_available, is_vision_available
...
if is_timm_available():
    ...
    from transformers import PrtrForObjectDetection, PrtrForSegmentation, PrtrModel


if is_vision_available():
    from PIL import Image

    from transformers import PrtrFeatureExtractor


class PrtrModelTester:
    def __init__(
        self,
        parent,
        batch_size=8,
    ...
    def create_and_check_prtr_model(self, config, pixel_values, pixel_mask, labels):
        model = PrtrModel(config=config)
    ...
    def create_and_check_prtr_object_detection_head_model(self, config, pixel_values, pixel_mask, labels):
        model = PrtrForObjectDetection(config=config)
        model.to(torch_device)
        model.eval()
...
@require_timm
class PrtrModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (
        (
            PrtrModel,
            PrtrForObjectDetection,
            PrtrForSegmentation,
        )
        if is_timm_available()
        else ()
    ...
    def _prepare_for_class(self, inputs_dict, model_class, return_labels=False):
        ...
        if return_labels:
            if model_class.__name__ in ["PrtrForObjectDetection", "PrtrForSegmentation"]:
                labels = []
    ...
    def setUp(self):
        self.model_tester = PrtrModelTester(self)
        self.config_tester = ConfigTester(self, config_class=PrtrConfig, has_text_modality=False)

    def test_config(self):
        self.config_tester.run_common_tests()

    def test_prtr_model(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_prtr_model(*config_and_inputs)

    def test_prtr_object_detection_head_model(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_prtr_object_detection_head_model(*config_and_inputs)

    @unittest.skip(reason="PRTR does not use inputs_embeds")
    def test_inputs_embeds(self):
        pass

    @unittest.skip(reason="PRTR does not have a get_input_embeddings method")
    def test_model_common_attributes(self):
        pass

    @unittest.skip(reason="PRTR is not a generative model")
    def test_generate_without_input_ids(self):
        pass

    @unittest.skip(reason="PRTR does not use token embeddings")
    ...
    def test_attention_outputs(self):
        ...
        for model_class in self.all_model_classes:
            ...
            if self.is_encoder_decoder:
                ...
                if model_class.__name__ == "PrtrForObjectDetection":
                    correct_outlen += 2
                # Panoptic Segmentation model returns pred_logits, pred_boxes, pred_masks
                if model_class.__name__ == "PrtrForSegmentation":
                    correct_outlen += 3
...
@require_vision
@slow
class PrtrModelIntegrationTests(unittest.TestCase):
    @cached_property
    def default_feature_extractor(self):
        return PrtrFeatureExtractor.from_pretrained("anugunj/prtr-hrtnet-coco") if is_vision_available() else None

    def test_inference_no_head(self):
        model = PrtrModel.from_pretrained("anugunj/prtr-hrtnet-coco").to(torch_device)
    ...
    def test_inference_object_detection_head(self):
        model = PrtrForObjectDetection.from_pretrained("anugunj/prtr-hrtnet-coco").to(torch_device)
    ...
    def test_inference_panoptic_segmentation_head(self):
        model = PrtrForSegmentation.from_pretrained("anugunj/prtr-hrtnet-coco-panoptic").to(torch_device)
